...
class IndoorDemoViewController: UIViewController, BMKMapViewDelegate, BMKPoiSearchDelegate, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell!.floorTitleLabel.text = _indoorMapInfoFocused?.arrStrFloors[(indexPath as NSIndexPath).row] as? String
        return cell!
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        //进行楼层切换
        let error = _mapView.switchBaseIndoorMapFloor(_indoorMapInfoFocused.arrStrFloors[(indexPath as NSIndexPath).row] as! String, withID: _indoorMapInfoFocused.strID)
    ...
    func mapview(_ mapView: BMKMapView!, baseIndoorMapWithIn flag: Bool, baseIndoorMapInfo info: BMKBaseIndoorMapInfo!) {
        ...
        if flag {//进入室内图
            if info != nil && info.arrStrFloors.count > 0 {
                ...
                let index = info.arrStrFloors.index(of: info.strFloor)
     ...
     */
    func onGetPoiIndoorResult(_ searcher: BMKPoiSearch!, result poiIndoorResult: BMKPoiIndoorResult!, errorCode: BMKSearchErrorCode) {
