    ...
    */
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if tableView == cityListTableView {
            ...
            if (indexPath as NSIndexPath).section == 0 {
                let item = hotCityData[(indexPath as NSIndexPath).row] as BMKOLSearchRecord
                ...
                let item = offlineCityData[(indexPath as NSIndexPath).row] as BMKOLSearchRecord
                cell?.textLabel?.text = "\(item.cityName)(\(item.cityID))"
                // 转换包大小
                let packSize = getDataSizeString(item.size)
                let sizeLabel = UILabel(frame: CGRect(x: 250, y: 0, width: 60, height: 40))
            ...
            if localDownloadMapInfo != nil  {
                let item = localDownloadMapInfo[(indexPath as NSIndexPath).row] as BMKOLUpdateElement
                // 单元格右侧文字
                let packSize = getDataSizeString(item.size)
                ...
                if item.ratio == 100{
                    ...
                    let currentSize = getDataSizeString(item.size / 100 * item.ratio)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if tableView == downloadTableView {
            ...
            if (indexPath as NSIndexPath).section == 0 {
                let item = hotCityData[(indexPath as NSIndexPath).row] as BMKOLSearchRecord
                print("热门城市：\(item.cityName)(\(item.cityID))--包大小：\(getDataSizeString(item.size))")
                cityIDLabel.text = "\(item.cityID)"
            }else if (indexPath as NSIndexPath).section == 1 {
                let item = offlineCityData[(indexPath as NSIndexPath).row] as BMKOLSearchRecord
                // 显示子单元格
